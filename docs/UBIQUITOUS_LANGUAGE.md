...
| **EmailSendAttempt** | Added 2026-08-27. One row in `email_log` — an attempt, **not** a delivery. `outcome: sent` means the provider accepted the message; whether it reached an inbox is not recorded and would need Resend's webhooks. Never describe an `email_log` row as proof a guest received something. |
| **ReservedRequestCap** | Added 2026-08-27. `MAX_RESERVED_PER_CUSTOMER` (6) — the most `reserved` bookings one customer may hold at once, across every item. Distinct from any per-item or per-date conflict rule: this is a customer-level fact, checked before per-night validation, and blocks regardless of which dates were requested. Not a business promise that a guest MAY hold up to 6 — it is abuse protection with an arbitrary owner-chosen number, not a feature with its own name in guest-facing copy. |
| **DailySendLimit** | Added 2026-08-27. The circuit breaker at 80 recipients per resort-local day, above which `sendEmail` refuses to send until tomorrow. Counted in *recipients*, not send calls, since one admin alert to three admins is three deliveries. Distinct from **DailyWarnThreshold** (30), which only changes what the dashboard says and never stops anything. Both are runaway protection, not prioritisation — the breaker cannot distinguish a guest confirmation from a loop and blocks both alike. |
