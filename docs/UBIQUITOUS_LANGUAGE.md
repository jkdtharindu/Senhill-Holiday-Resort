...
| **CustomerCancellation** | Not supported. A Customer can view but never change their own Booking's status — moving a Booking out of `reserved` (to `declined`, or any future `cancelled` state) is admin-only. Confirmed via Grill Me. |
